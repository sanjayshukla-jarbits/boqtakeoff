                               ...
                               out T ix, out T iy)
{
  ...
  return true;
    ...
    {
        /*
        public static Definition externalDefinition(
         this Definition myDefinition, DefinitionGroup myGroup)
        {
            // create an instance definition in definition group SMI Parameters
            ExternalDefinitionCreationOptions option = new ExternalDefinitionCreationOptions("room_id", ParameterType.Text);
            // Don't let the user modify the value, only the API
            option.UserModifiable = false;
            // Set tooltip
            option.Description = "Room ID";
        ...
        */
        ...
        public static String GetFamilyTypeMarkParam(
            this FamilyInstance fyInst, Document document)
        {
            Parameter param = fyInst.Symbol.get_Parameter(BuiltInParameter.WINDOW_TYPE_ID);
            return param == null
                ? string.Empty
                : param.AsString();
        }

        public static bool GetRoomName(
            this FamilyInstance fyInst, string room_name)
        {
            Parameter param = fyInst.Symbol.get_Parameter(BuiltInParameter.ELEM_ROOM_NAME);

            if (param != null && !param.IsReadOnly && param.IsShared)
            {
                bool bRet = param.Set(room_name.ToString());
            }
        ...
        public static String GetLevelName(
            this FamilyInstance fyInst)
        {
            Parameter param = fyInst.Symbol.get_Parameter(BuiltInParameter.SCHEDULE_LEVEL_PARAM);
            return param == null
                ? string.Empty
                : param.AsString();
        }
        ...
        public static Parameter GetFamilyMarkParam(
            this FamilyInstance f)
        {
            var p = f.get_Parameter(
        ...
        public static bool SetFamilyMarkParam(
            this Parameter familyParameter, string setString)
        {
            // familyInstance.LookupParameter("MARK").Set(setString);
            Boolean setStatus = familyParameter.Set(setString);

            //            f.Set(famParam, "SANJAYSHUKLA");
        ...
        public static void AddParamToFamilyInstance(
            this FamilyInstance fyInst, Document document, String room_id)
        {

            TaskDialog.Show("Room Id", room_id.ToString());
            try
            {

                Family fam = fyInst.Symbol.Family;
                Document docfamily = document.EditFamily(fam);

                FamilyManager famMan = docfamily.FamilyManager;
                ...
                using (Transaction trans = new Transaction(document, "Change Parameter"))
                {
                    ...
                    {
                        FamilyParameter famParam = famMan.get_Parameter("SMI_room_id");
                        famMan.RemoveParameter(famParam);
                    }
                    ...
                    */
                    ...
                    {
                        //add the new parameter - last parameter is true for an instance parameter, false for a type parameter
                        // FamilyParameter famParam = famMan.AddParameter("room_id", BuiltInParameterGroup.PG_TEXT, ParameterType.Text, true);
                        FamilyParameter famParam = famMan.get_Parameter("SMI_room_id");
                        // ElementId etypeid = elem.GetTypeId();
                        // Element etype = document.GetElement(etypeid);
                        famMan.Set(famParam, room_id.ToString());
                        // Boolean setStatus = famParam.Set(room_id);
                    }
                    catch
                    {
                    }
                ...
                //load the family back in to the project
                using (Transaction trans = new Transaction(document, "Load Family"))
